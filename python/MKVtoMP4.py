...
def convert_video(input_file, output_file, encoding_profile):
    ...
    if encoding_profile == 'video_passthrough':
        ...
        call_parameters = [
            ...
            '-y',
            '-filter_complex',
            '[0:v][0:s]overlay,scale=w=min(1280\,iw):h=min(720\,ih):force_original_aspect_ratio=decrease,scale=w=trunc(iw/2)*2:h=trunc(ih/2)*2[scaled]',
            '-map',
            '[scaled]',
            '-map',
            '0:a',
        ...
        call_parameters = [
            ...
            '-y',
            '-filter_complex',
            '[0:v]scale=w=min(1280\,iw):h=min(720\,ih):force_original_aspect_ratio=decrease,scale=w=trunc(iw/2)*2:h=trunc(ih/2)*2[scaled]',
            '-map',
            '[scaled]',
            '-map',
            '0:a',
